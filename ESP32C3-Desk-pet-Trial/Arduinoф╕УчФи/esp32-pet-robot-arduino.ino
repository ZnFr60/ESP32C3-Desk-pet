...
void enterDeepSleep() {
    ...
    // Save trial state to flash before sleeping
    trialManager.saveToFlash();
...
void setup() {
    ...
    // Initialize Trial Manager (check lock state from flash)
    trialManager.begin();
    if (trialManager.isLocked()) {
        // Device is locked - show lock screen immediately and skip all
        // further initialization to save power (no MPU6050, no WiFi, etc.)
        display.clearDisplay();
        display.setTextSize(1);
        display.setTextColor(SSD1306_WHITE);
        ...
        display.print("please make a");
        display.setCursor(10, 50);
        display.print("decision");
        display.display();
        // Return early: loop() will handle lock screen display
        // All hardware init (MPU6050, WiFi, TimeManager, RoboEyes) is skipped
        return;
    }
...
void loop() {
    // ============================================================
    // Trial Manager: update usage timer and check lock state
    // ============================================================
    trialManager.update();
    
    // If locked, show lock screen and block all functionality
    if (trialManager.isLocked()) {
        // Re-draw lock screen periodically to ensure it stays visible
        static unsigned long lastLockDraw = 0;
        if (millis() - lastLockDraw > 2000) {
            display.clearDisplay();
            display.setTextSize(1);
            display.setTextColor(SSD1306_WHITE);
            display.setCursor(8, 18);
