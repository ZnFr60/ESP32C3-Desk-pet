...
// Forward declaration (full definition is placed just before setup()).
// Keeps the OLED animating during blocking WiFi/NTP operations.
void eyesTick();

// ============================================================
// I2C Scanner + auto-address detection + redundancy
// ============================================================
// Auto-detected addresses (0 = not found). Populated by scanI2C().
uint8_t detectedOLEDAddr = 0;
uint8_t detectedMPUAddr  = 0;
// Runtime OLED address; defaults to SCREEN_ADDRESS but is updated from the
// scan (0x3C vs 0x3D). RoboEyesManager uses this instead of the fixed macro
// so its contrast command always targets the device actually on the bus.
uint8_t oledAddress = SCREEN_ADDRESS;

// Scans the I2C bus, records the SSD1306 (0x3C/0x3D) and MPU6050 (0x68/0x69)
// addresses it actually finds, and returns true if any device responded.
bool scanI2C() {
    byte error, address;
    int nDevices = 0;
    detectedOLEDAddr = 0;
    ...
    for (address = 1; address < 127; address++) {
        ...
        if (error == 0) {
            ...
            if (address == 0x3C || address == 0x3D) { Serial.print(" (SSD1306 OLED)"); detectedOLEDAddr = address; }
            else if (address == 0x68 || address == 0x69) { Serial.print(" (MPU6050)"); detectedMPUAddr = address; }
            Serial.println();
            nDevices++;
        }
    }

    if (nDevices == 0) { Serial.println("  No I2C devices found!"); return false; }
...
void syncNTPAndDisconnect() {
    ...
    bool synced = timeManager.syncFromNTP(eyesTick);
...
void handleMidnightSync() {
    ...
    bool connected = wifiManager.tryAutoConnect(WIFI_SEARCH_TIMEOUT_MS, eyesTick);
...
// ============================================================
// Screen tick during blocking operations (WiFi/NTP)
// ============================================================
// Keeps the OLED animating while the code waits for WiFi/NTP so the screen is
// never left black. Safe to call from within tryAutoConnect()/syncFromNTP().
void eyesTick() {
    roboEyesManager.update();
    if (roboEyesManager.wasFrameDrawn()) {
        roboEyesManager.drawOverlays(display, timeManager.hasValidTime());
    }
}
...
void setup() {
    ...
    // ============================================================
    // I2C bus: start at 400kHz; if nothing is found, drop to 100kHz and
    // rescan. Long wires / weak pull-ups often fail at fast mode, so this
    // slow-mode retry is cheap redundancy that cures many "black screen"
    // cases where the OLED is wired correctly but can't talk at 400kHz.
    // ============================================================
    Wire.begin(I2C_SDA_PIN, I2C_SCL_PIN);
    Serial.printf("[I2C] begin(): SDA=%d SCL=%d driver_installed=%d\n",
        Wire.sdaPin(), Wire.sclPin(), Wire.isInstalled());
    Wire.setClock(400000);
    Serial.printf("I2C init (SDA=%d, SCL=%d) @ 400kHz\n", I2C_SDA_PIN, I2C_SCL_PIN);
    bool busOK = scanI2C();
    if (!busOK) {
        Serial.println("No devices at 400kHz -> retrying at 100kHz (slow mode)...");
        Wire.setClock(100000);
        busOK = scanI2C();
    }

    // ============================================================
    // OLED: init using the AUTO-DETECTED address (0x3C or 0x3D).
    //
    // IMPORTANT: Adafruit_SSD1306::begin() does NOT probe the I2C bus and
    // typically returns true even when no OLED is attached. So the only
    // reliable "device is really here" signal is the scan above. We use the
    // scan both to pick the correct address and to decide screen presence.
    // ============================================================
    Serial.println("Initializing OLED...");
    oledAddress = (detectedOLEDAddr != 0) ? detectedOLEDAddr : SCREEN_ADDRESS;
    if (detectedOLEDAddr != 0) Serial.printf("OLED detected at 0x%02X\n", oledAddress);
    else                       Serial.printf("OLED not in scan -> trying configured 0x%02X\n", SCREEN_ADDRESS & 0x7F);

    // Robust OLED bring-up. begin() performs a hardware reset now that
    // OLED_RESET is a real GPIO (was -1 => never reset, a black-screen cause).
    // Force DISPLAYON + contrast afterwards so a one-shot init hiccup cannot
    // leave the panel dark.
    // Explicit SSD1306 hardware reset (active-low), independent of the library.
    ...
    bool oledOK = (detectedOLEDAddr != 0);   // trust the scan, NOT begin()
    if (oledOK) {
        display.setTextSize(1);
        display.setTextColor(SSD1306_WHITE);
        display.clearDisplay();
        ...
        Serial.println("    Check SDA(GPIO3)/SCL(GPIO4) wiring, 3.3V power and pull-ups.");
        // Visible failure signal: 3 bursts of 3 short blinks on built-in LED.
        // Do NOT hard-hang here — a frozen board looks identical to a dead
        // board. Keep running so Serial logs and GPIO events still work.
    ...
    // Initialize MPU6050 (auto-detected address 0x68/0x69, with fallback)
    Serial.println("Initializing MPU6050...");
    uint8_t mpuAddr = (detectedMPUAddr != 0) ? detectedMPUAddr : 0x68;
    uint8_t mpuAlt  = (mpuAddr == 0x68) ? 0x69 : 0x68;
    bool mpuOK = mpu.begin(mpuAddr, &Wire);
    if (!mpuOK) {
        Serial.printf("MPU6050 not at 0x%02X -> trying 0x%02X\n", mpuAddr, mpuAlt);
        mpuOK = mpu.begin(mpuAlt, &Wire);
    }
    if (mpuOK) {
    ...
    // ============================================================
    // Initialize RoboEyes EARLY so the screen is never black
    // ============================================================
    // The eyes must start animating BEFORE any blocking WiFi/NTP work below,
    // otherwise the OLED stays blank for the full WiFi timeout (~60s) on a
    // fresh power-on. This was the primary cause of the "black screen".
    ...
    // Draw the first eye frame now so the display is live immediately
    eyesTick();
    ...
    //
    // NOTE: eyesTick() keeps the screen animating during any blocking
    // connection attempt, so the device always looks alive.
    // ============================================================
