...
public:
    ...
    //
    // `onTick`, if provided, is called while waiting so the caller can keep the
    // screen animating (prevents a black/frozen screen during long connections).
    bool tryAutoConnect(unsigned long timeoutMs, void (*onTick)() = nullptr) {
