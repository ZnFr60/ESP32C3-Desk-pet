...
// Display colors
// NOTE: `static` gives these internal linkage so including this header in more
// than one translation unit does not cause "multiple definition" link errors.
static uint8_t BGCOLOR = 0; // background and overlays
static uint8_t MAINCOLOR = 1; // drawings

// For mood type switch (prefixed to avoid collisions with Arduino/stdlib)
...
#define ROBO_N 1 // north, top center
#define ROBO_NE 2 // north-east, top right
#define ROBO_E 3 // east, middle right
#define ROBO_SE 4 // south-east, bottom right
#define ROBO_S 5 // south, bottom center
#define ROBO_SW 6 // south-west, bottom left
#define ROBO_W 7 // west, middle left
#define ROBO_NW 8 // north-west, top left
...
// When true, drawEyes() skips display->display() to allow overlays to be drawn first
// This eliminates double-refresh and doubles the effective frame rate
bool skipAutoDisplay = false;
...
// Space between eyes
// (declared BEFORE the coordinate defaults below, which depend on this value;
// declaring it after them caused undefined behaviour / wrong default positions)
...
void setFramerate(byte fps){
  if (fps < 1) fps = 30;   // guard against divide-by-zero / invalid saved setting
  if (fps > 60) fps = 60;  // sane upper bound for an SSD1306 OLED
...
void drawEyes(){
  ...
  if(!skipAutoDisplay) display->display(); // show drawings on display (unless caller will draw overlays first)
  // If skipAutoDisplay is true, the caller is responsible for calling display->display()
  // after drawing overlays, eliminating the double-refresh performance issue
